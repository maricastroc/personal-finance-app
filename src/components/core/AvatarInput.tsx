...
export const AvatarInput = ({
  ...
  return (
    <div className="flex flex-col w-full sm:mb-4 overflow-hidden sm:grid sm:grid-cols-[1fr,3.6fr] items-center gap-4">
      <div className="flex items-center justify-center border-2 border-surface-600 rounded-full w-30 h-30">
        <div className="flex items-center justify-center relative h-[6.5rem] min-w-[6.5rem]">
          {avatarPreview ? (
            ...
            <User className="w-16 h-16 text-ink-300" aria-hidden="true" />
          )}
          ...
          <button
            ...
            className="absolute hover:bg-grey-500 focus:ring-2 focus:ring-medium-purple cursor-pointer top-[73%] left-[72%] flex items-center justify-center bg-surface-700 border border-surface-600 rounded-full text-white w-[2.2rem] h-[2.2rem] sm:left-[68%]"
          >
            <Pencil size={18} />
          </button>
        </div>
      ...
      <div className="flex flex-col flex-grow w-full">
        <label
          htmlFor="avatar"
          className="text-xs font-bold text-ink-300 mb-1 cursor-pointer"
        >
          Avatar
        </label>
